...
public class CmdLineParser {
    ...
    public final Option<String> addPasswordOption( String shortForm, String longForm, boolean required) {
        return addOption(new Option.PasswordOption(shortForm, longForm, required));
    }
    ...
    public final Option<String> addPasswordOption( String longForm, boolean required ) {
        return addOption(new Option.PasswordOption(longForm, required));
    }
    ...
    public void printUsage(String packageName, String feature) {
        ...
        FeatureUtil.logMsg("\r\nUSAGE: java -jar oracle-db-tool.jar " + packageName + " " + feature + " [arguments]");

        if(optionHelpStringsRequired.size() > 0)
            FeatureUtil.logMsg("\r\nREQUIRED ARGUMENTS:\r\n");

        for (String help : optionHelpStringsRequired) {
            FeatureUtil.logMsg(help);
        }
    ...
    public void printDescription(boolean isOneliner)
    {
        if(description != null && description.length() > 0) {
            if(!isOneliner) {
                FeatureUtil.logMsg("\r\nDESCRIPTION:\r\n");
                FeatureUtil.logMsg(description);
            } else {
                FeatureUtil.logMsg(" - " + description);
            }
        }
    ...
    public void printParams() {
        ...
        for ( String argument : options.keySet() ) {

            if ( !(options.get(argument) instanceof Option.PasswordOption) ) {
                String key = options.get(argument).longForm();

                if (! isPrinted.add(key))
                    continue;  // Prevent the info being showed twice (long form, short form)
                ...
                FeatureUtil.logMsg(key + " = " + value, MsgTypes.PARAM);
            }
        }
    }
